            ...
            {testimonials.map(({ text, image, name, role }, i) => (
              ...
              >
                ...
                <blockquote className="m-0 p-0">
                  <p className="text-[#2C2100] text-sm leading-relaxed font-light group-hover:text-[#0D0D0D] transition-colors duration-300">
                    {text}
                  </p>
                  <footer className="flex items-center gap-3 mt-5 pt-5 border-t border-[#C4A010]/35">
                    <img
                      width={36}
                      height={36}
                      src={image}
                      alt={`Avatar of ${name}`}
                      className="h-9 w-9 rounded-full object-cover ring-1 ring-[#8C7A3A] group-hover:ring-[#C62B1E]/40 transition-all duration-300"
                    />
                    <div className="flex flex-col">
                      <cite className="font-black not-italic text-xs text-[#0D0D0D] uppercase tracking-[0.1em]">
                        {name}
                      </cite>
                      <span className="text-[11px] text-[#6B5820] mt-0.5 font-medium">
                        {role}
                      </span>
                    </div>
                  </footer>
